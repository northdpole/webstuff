...
class vegas{
 ...
 public function create_vegas_admin_page(){
	...
	register_setting('vegas_option_group', 'where_key', array($this, 'check_where'));
  ...
  add_settings_field(
	...
	add_settings_field(
	    'where',
      'where (id):',
	    array($this, 'where_field'),
	    'vegas-setting-admin',
	    'setting_section_id'
	);
    }
	public function check_where($input){
		var_dump("Where_Check");
        if( isset($input['where'])){$where = $input['where'];}

        if( !isset($where['where']) && isset($input['where'])){
	    if(get_option('where') === FALSE){
		add_option('where', $where);
	    }else{
		update_option('where', $where);
	    }
	}else{
	    $where = '';
	}
...
public function check_overlay($input){
  ...
  public function check_title($input){
   ...
   return $Global;
    ...
    public function where_field(){
        ?><input type="text" id="where" name="where_key[where]" value="<?php echo get_option('where'); ?>" /><?php
    }
